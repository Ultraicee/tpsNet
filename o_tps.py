...
parser.add_argument('--batch_size',               type=int, help='num of minimum batch size', default=20)
...
with tf.Graph().as_default():
    ...
    # loss for training T
    loss_rec1, loss_smooth1, compa_sum1, _ = compute_smooth_rec_loss(right_est1, right, compensateI,
                                                                linear_interpolator.sz_params, T_weight_true,
                                                                T_weight)
    # calculate batch loss and single image loss
    loss_rec2, loss_smooth2, compa_sum2, _ = compute_smooth_rec_loss(right_est2, right, compensateI,
                                                                linear_interpolator.sz_params, T_weight_true,
                                                                T_weight)

    # loss for training theta
    loss_rec3, loss_smooth3,  compa_sum3, loss_rec3_per = compute_smooth_rec_loss(right_est2, right, compensateI,
                                                                linear_interpolator.sz_params,T_weight_true,T_weight,
                                                                 "batch")
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(params.epoch_num): # period of entire train T and theta
            ...
            for step in range(0, params.max_step):
                ...
                for batch_idx in range(batch_num):
                    ...
                    theta_val, T_val, loss_rec_val, loss_smooth_val, loss_norm_val, compa_sum3_val, disp_val, est_right_val = sess.run(
                            [theta_input, T_weight, loss_rec3_per, loss_smooth3, loss_wt_norm,compa_sum3, disp2,right_est2],
                        feed_dict={theta_input_f: cur_theta, T_weight_f: out_T, left: cur_left, right: cur_right,
                                   theta_in_base: theta_baseline, disp_base: disp_baseline, disp_True: cur_disp})
                    ...
                    g_step = step*batch_num+batch_idx
                    if g_step % 10 == 0 and g_step > 0:
                        print(
                            print_str.format(g_step, loss_rec_val_mean, loss_smooth_val_mean, loss_norm_val_mean, loss_val_mean))  # loss message of current patch
            ...
            print("During training T, is theta keep constant?")
            if theta_val.any() == cur_theta.any():
                print("True")
            else:
                print("False")
            """ train T close """

            # update theta and T for next theta training
            for batch_idx in range(batch_num):
                ...
                sess.run([update1, update2], feed_dict={theta_input_f: cur_theta, T_weight_f: out_T})
            ...
            for step in range(0, params.max_step):
                for batch_idx in range(batch_num):
                    ...
                    g_step = step * batch_num + batch_idx
                    if g_step + 1 == params.max_step*batch_num or (g_step % 10 == 0 and g_step > 0):
                        print(
                            print_str.format(g_step, params.cpts_row * params.cpts_col * loss_rec_val / compa_sum3_val,
                                             loss_smooth_val, loss_norm_val,
                                             loss_val))
                        loss_rec_var = np.abs(loss_rec_val - loss_rec_temp)
                        loss_rec_temp = loss_rec_val
                        if step >= 0.6*params.max_step:
                    ...
                    theta_batch = np.copy(theta_val)  # update theta each batch
                # update iterative value each step
                trained_disp = np.copy(out_disp)
                trained_theta = np.copy(out_theta)
            print("During training T, is theta keep constant?")
            if out_T.any() == T_val.any():
                print("True")
            else:
                print("False")
            print("[%d / %d] finished training of z" % (i + 1, params.epoch_num))
